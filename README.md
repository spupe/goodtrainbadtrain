...
We gathered records of all high-speed train rides between Cologne, Munich and Berlin in 2020 and 2021 and combined it with historical weather information for each ride. Then, we encoded real-world information in a way that could be understood by a machine learning model - for example, we filtered periods with lockdowns, flagged all major holidays and added variables indicating the week day of a given ride, as all these factors could be relevant for the model's performance.

On a held-out test set, the retrained classifier (LightGBM with class weighting, target = delay > 15 min) reaches **ROC-AUC ≈ 0.81** and correctly flags **~65% of delayed trains** at 37% precision (overall accuracy 0.77, vs. a "predict on time always" baseline of 0.84). Earlier versions of this README cited ~80% accuracy; that number was inflated by a leakage bug in the original training pipeline. See [`notebooks/EXPERIMENTS.md`](notebooks/EXPERIMENTS.md) for the full retraining log, ablations, and reasoning behind the chosen operating point.
